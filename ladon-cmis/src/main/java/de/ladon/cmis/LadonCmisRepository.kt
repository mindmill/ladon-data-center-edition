...
class LadonCmisRepository(
    ...
    private val repositoryInfo10: RepositoryInfo
    /** CMIS 1.1 repository info.  */
    private val repositoryInfo11: RepositoryInfo

    init {
        // check repository id
        if (repositoryId == null || repositoryId.trim { it <= ' ' }.length == 0) {
    ...
    private fun createRepositoryInfo(cmisVersion: CmisVersion): RepositoryInfo {
        ...
        repositoryInfo.cmisVersionSupported = cmisVersion.value()

        repositoryInfo.productName = "OpenCMIS FileShare"
        repositoryInfo.productVersion = "1.1.0"
        repositoryInfo.vendorName = "OpenCMIS"
    ...
    fun getRepositoryInfo(context: CallContext): RepositoryInfo {
        ...
        return if (context.cmisVersion == CmisVersion.CMIS_1_0) {
            repositoryInfo10
        } else {
            repositoryInfo11
        }
