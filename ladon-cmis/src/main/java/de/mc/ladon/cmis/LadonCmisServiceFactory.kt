...
class LadonCmisServiceFactory : AbstractServiceFactory() {
    ...
    override fun getService(context: CallContext): CmisService {
        ...
        val user = try {
            LadonServicesHolder.userDetailsManager().loadUserByUsername(context.username)
        } catch (e: LadonObjectNotFoundException) {
            throw CmisPermissionDeniedException("Invalid username or password.")
        }
        if (!BCryptPasswordEncoder().matches(context.password,user.password) || !user.isEnabled)
            throw CmisPermissionDeniedException("Invalid username or password.")
